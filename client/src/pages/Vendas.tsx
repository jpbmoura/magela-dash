...
export default function Vendas() {
  ...
  const chartCategoria = (vendasPorCategoria || [])
    .slice(0, 6)
    .map((d: any) => ({
        ...
        {[
          {
            ...
            value: formatCurrencyShort(parseFloat(String(metrics?.total ?? 0))),
            Icon: DollarSign,
          ...
          {
            label: "Qtd. de Vendas",
            value: (metrics?.quantidade ?? 0).toLocaleString("pt-BR"),
            Icon: ShoppingCart,
          ...
          <div
            key={kpi.label}
            className="relative rounded-xl border bg-card px-5 py-4 shadow-sm overflow-hidden"
          >
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div
                className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${kpi.bg}`}
              >
                ...
                <div className="space-y-2">
                  {topProdutos.map((p: any, i: number) => {
                    const maxVal = parseFloat(
                      (topProdutos[0] as any)?.faturamento || 1
                    );
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-0.5">
                            <span className="text-sm font-medium truncate max-w-[300px]">
                              {p.produto || p.codProduto}
                            </span>
                            <span className="text-sm font-semibold text-primary ml-2">
                              {formatCurrency((p as any).faturamento)}
                            </span>
                          </div>
                  ...
                  vendasPaginadas.data.map((v: any) => (
                    <tr key={v.id}>
                      <td className="text-muted-foreground text-xs whitespace-nowrap">
                        {formatDate(v.emissaoData)}
                      </td>
                      <td className="max-w-[160px] truncate font-medium">
                        {v.razaoSocial || v.cliente || "—"}
                      </td>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t bg-muted/10">
              <p className="text-xs text-muted-foreground">
                ...
                {vendasPaginadas?.total?.toLocaleString("pt-BR")} registros
              </p>
